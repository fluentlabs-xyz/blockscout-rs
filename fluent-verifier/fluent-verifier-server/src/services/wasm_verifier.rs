...
impl WasmVerifier for FluentWasmVerifierService {
    ...
    async fn list_available_versions(
        ...
        let sdk_versions = list_docker_image_tags(request_inner.include_prerelease).await
            .map_err(|e| {
...
fn validate_verification_request(request: &VerifyWasmRequest) -> anyhow::Result<()> {
    ...
    if compile_settings.sdk_version.is_empty() {
        return Err(anyhow::anyhow!("sdk_version is required in compile_settings"));
    }

    // Validate contract address format (basic check)
    if !request.contract_address.starts_with("0x") || request.contract_address.len() != 42 {
        return Err(anyhow::anyhow!("contract_address must be a valid Ethereum address (0x + 40 hex chars)"));
    }
...
async fn list_docker_image_tags(
    ...
    let tags_response = client.get(tags_url)
    ...
    if !include_prerelease {
        ...
        tags.tags.retain(|v| {
            v == "latest" || (!v.contains('-') && v.starts_with('v'))
        });
    }
...
fn find_latest_stable_version(versions: &[String]) -> String {
    ...
    let mut stable_versions: Vec<&String> = versions.iter()
    ...
    stable_versions.sort_by(|a, b| {
        ...
        let a_parts: Vec<u32> = a[1..].split('.')
            .filter_map(|s| s.parse().ok())
            .collect();
        let b_parts: Vec<u32> = b[1..].split('.')
            .filter_map(|s| s.parse().ok())
            .collect();
